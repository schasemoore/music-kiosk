...
| `flipFly(fromRect, toRect, photoSrc, themeColor, onLanded, opts)` | 397 | The pop-out mechanism: builds a fixed-position clone and animates its real `left`/`top`/`width`/`height` (not `transform: scale`, which stretched the photo — see `reference/decisions-log.md`) between the tile's rect and the viewport via the Web Animations API, so `object-fit: cover` on the inner photo recomputes natively every frame. `opts = { tileEl, mode: "open"|"close", tilePhotoSrc }` makes the clone start/end as an exact visual copy of the tile: the photo layer is placed at the tile photo's *actual current* rect (the tile photo is a 110%-sized, drifting Ken Burns layer, not a plain cover crop) and the tile's scrim/arrow/name/tagline are copied and faded out (open) or back in (close); on close, if the visitor swiped to a different slide, the tile's own photo crossfades in. 520ms, `cubic-bezier(.4,0,.2,1)`. Used by `openPreview`/`openStudio` and `closePreview`/`closeStudio` |
...
| `positionHotspots()` / `studioCoverMetrics()` | ~608 / ~604 | The studio photo is full-bleed `object-fit: cover` like every other tile, so a hotspot's raw x/y% (a point on the *original* photo) is re-mapped through the cover scale-and-align math (using `studioPan` as the `object-position` fraction) to its on-screen position. Runs on photo load, resize, and every pan. Also swaps the intro line to a "swipe to look around" variant when the crop hides >12% of the width, and toggles each hotspot's `.flip` from its *on-screen* x |
...
| `openStudio()` / `closeStudio()` | ~688 / ~715 | The Lucky Man Studio tile's open/close — same `flipFly` pop-out and same `.info-visible` reveal/dismiss timing as `openPreview`/`closePreview`, and `#studio-view` uses the same `.preview-modal`/`.preview-media`/`.preview-info` classes (close button, full-bleed cover photo, bottom eyebrow/name/tagline scrim, Apply button from the site-wide `cta`) — the differences are the hotspot layer over the photo, an actionable intro line instead of the tile's description, and no "More about this program" (there's no separate studio page). Photo + hotspots are painted into the still-hidden overlay *before* the pop-out starts (mirrors `openPreview`'s hand-off trick); `studioPan` resets to centered on open so the crop matches the clone's at hand-off. Drag-to-pan (pointer events on `#studio-media`) reveals the cropped-out parts of the photo |
...
`studio-tile` / `studio-tile-open` (the Lucky Man Studio hub tile — built in `renderHub()` alongside the `areas[]` tiles but not one of them, no `data-area` attribute), `studio-view` (a `.preview-modal` — same classes as `area-preview`, on purpose) / `studio-media` (the full-bleed, drag-to-pan photo container) / `studio-photo` / `hotspot-layer` / `studio-eyebrow` / `studio-name` / `studio-tagline` / `studio-apply` / `studio-close`.
